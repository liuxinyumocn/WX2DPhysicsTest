    ...
    */
    initPhysics(){
        ...
        //创建三个地面
        {
            ...
            groundBody.SetTransform(new Box2D.b2Vec2(systemInfo.windowWidth,-1*systemInfo.windowHeight * 2),0);
            let groundBox = new Box2D.b2PolygonShape();
            groundBox.Set(systemInfo.windowWidth,-1*systemInfo.windowHeight * 2);
            groundBox.SetAsBox(systemInfo.windowWidth,10);
        ...
        }
        {
            ...
            let groundBody = this.world.CreateBody(groundBodyDef);
            groundBody.SetTransform(new Box2D.b2Vec2(0,-1*systemInfo.windowHeight * 2),0);
            let groundBox = new Box2D.b2PolygonShape();
            groundBox.SetAsBox(10,systemInfo.windowHeight);
            let groundFixtureDef = new Box2D.b2FixtureDef();
            groundFixtureDef.shape = groundBox;
            groundBody.CreateFixture(groundFixtureDef);
            console.log(groundBody.GetPosition());   
        }
        {
            //右面
            let groundBodyDef = new Box2D.b2BodyDef();
            ...
            let groundBody = this.world.CreateBody(groundBodyDef);
            groundBody.SetTransform(new Box2D.b2Vec2(systemInfo.windowWidth*2,-1*systemInfo.windowHeight),0);
            let groundBox = new Box2D.b2PolygonShape();
            groundBox.SetAsBox(10,systemInfo.windowHeight);
            let groundFixtureDef = new Box2D.b2FixtureDef();
            groundFixtureDef.shape = groundBox;
    ...
    genPhyRect(x,y,w,h){
        ...
        obFixtureDef.shape = dynamicBox;
        obFixtureDef.density = 2.5;
        obFixtureDef.restitution = 0.9;
        obFixtureDef.friction = 0.6;
